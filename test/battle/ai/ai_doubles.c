...
AI_DOUBLE_BATTLE_TEST("AI Buizel uses Aqua Jet to finish 1 HP Combusken")
{
    PASSES_RANDOMLY(100, 100);
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT);
        PLAYER(SPECIES_COMBUSKEN) { Level(20); HP(1); Ability(ABILITY_SPEED_BOOST); Moves(MOVE_DETECT, MOVE_DOUBLE_KICK, MOVE_AERIAL_ACE, MOVE_FLAME_CHARGE); }
        PLAYER(SPECIES_QUILAVA) { Level(20); Ability(ABILITY_ADAPTABILITY); Moves(MOVE_MUD_SHOT, MOVE_INCINERATE, MOVE_QUICK_ATTACK, MOVE_LEER); }
